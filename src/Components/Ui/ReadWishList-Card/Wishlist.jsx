...
const Wishlist = ({ sortingType }) => {
    const { wishlistBook } = use(bookContext);

    const [filterReadList, setFilterReadList] = useState(wishlistBook);
    useEffect(() => {
        if (sortingType) {
            if (sortingType === "pages") {
                ...
                const sortedData = [...wishlistBook].sort((a, b) => a.rating - b.rating)
                setFilterReadList(sortedData);
            }
        }
        ...
        <div>
            {
                filterReadList.map(bookdata => (
                    ...
                    <div className="flex mt-[60px] gap-[48px] container mx-auto items-center border-0 shadow p-4 rounded-2xl">
